...
class Question{
	...
	public function addNewQuestion($questionParams){
		$user = new Person($this->pdo);
		$userID = $user->getUserID($questionParams['name'], $questionParams['email']);
		$dt = date("Y-m-d H:i:s", time());
        $stmt = $this->pdo->prepare("INSERT INTO questions (q_text, id_user, id_category, q_date) VALUES (:qtext, :iduser, :idcategory, :qdate)");
	...
	public function deleteByCategory($catID){
		$stmt = $this->pdo->prepare("DELETE FROM questions WHERE id_category = :id");
        $stmt->execute(array(':id' => $catID));
	}
	public function selectByCategoryAndStatus($catID, $status){
		$stmt = $this->pdo->prepare("SELECT questions.q_text, answers.a_text 
	...
	public function selectByStatus($status){
		$stmt = $this->pdo->prepare("SELECT questions.id, questions.q_text, categories.title, questions.q_date
	...
	public function editStatus($id, $status){
		$stmt = $this->pdo->prepare("UPDATE questions SET status = :status WHERE id = :id");
		$stmt->execute(array(':status' => $status, ':id' => $id));
	}
	public function get($id){
		$stmt = $this->pdo->prepare("SELECT questions.id as qid, questions.q_text, users.id as user, answers.id as aid, answers.a_text 
	...
	public function select($id){
		$stmt = $this->pdo->prepare("SELECT id, q_text, id_user, q_date, status FROM questions WHERE id_category=:id_cat");
	...
	public function updateText($id, $text){
		$stmt = $this->pdo->prepare("UPDATE questions SET q_text = :text WHERE id = :id");
